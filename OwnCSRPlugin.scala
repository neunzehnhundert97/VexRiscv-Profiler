...
import vexriscv.plugin.HazardService
import vexriscv.plugin.{CsrInterface, CsrPluginConfig, IContextSwitching, IWake, CsrAccess, ExceptionPortInfo, CsrMapping, _}
...
class CsrPlugin(val config: CsrPluginConfig)
    ...
    with ExceptionService
    with PrivilegeService
    with InterruptionInhibitor
    with ExceptionInhibitor
    with IContextSwitching
    with CsrInterface
    with IWake {
  import config._
  import CsrAccess._
...
//      timerInterruptS    = in Bool() setName("timerInterruptS")
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    val trapCodeWidth =
      log2Up((List(16) ++ interruptSpecs.map(_.id + 1) ++ exceptionPortsInfos.map(p => 1 << widthOf(p.port.code))).max)
    ...
    val machineCsr = pipeline plug new Area {
      ...
      val mscratch = if (mscratchGen) Reg(Bits(xlen bits)).addTag(Verilator.public) else null
      val mcause = new Area {
      ...
      val medeleg = supervisorGen generate new Area {
        ...
        val mapping = mutable.LinkedHashMap(
          0 -> IAM,
          1 -> IAF,
          2 -> II,
          4 -> LAM,
    ...
    pipeline plug new Area {
      ...
      val exceptionPortCtrl = exceptionPortsInfos.nonEmpty generate new Area {
        ...
        val exceptionValidsRegs =
          Vec(stages.map(s => Reg(Bool).init(False).setPartialName(s.getName()))).allowUnsetRegToAvoidLatch
...
//        sortedByStage.zipWithIndex.foreach(e => e._1.port.setName(e._1.stage.getName() + "_exception_agregat"))
        ...
        for (stageId <- firstStageIndexWithExceptionPort until stages.length; stage = stages(stageId)) {
          ...
          when(!stage.arbitration.isStuck) {
            exceptionValidsRegs(stageId) := (if (stageId != firstStageIndexWithExceptionPort)
                                               exceptionValids(stageId - 1) && !previousStage.arbitration.isStuck
                                             else False)
          } otherwise {
        ...
        while (privilegs.nonEmpty) {
          ...
          when(privilegeAllowInterrupts(p)) {
            for (
              ...
              if privilegs.tail.forall(e => i.delegators.exists(_.privilege == e))
            ) { // EX : Supervisor timer need to have machine mode delegator
      ...
      val lastStageWasWfi =
        if (wfiGenAsWait) RegNext(lastStage.arbitration.isFiring && lastStage.input(ENV_CTRL) === EnvCtrlEnum.WFI) init (False)
        else False
...
//        val pcValids = for(stage <- stagesFromExecute) yield RegInit(False) clearWhen(!started) setWhen(!stage.arbitration.isValid)
      ...
      when(hadException || interruptJump) {
        ...
        jumpInterface.payload := (if (!xtvecModeGen) xtvec.base @@ U"00"
                                  else
                                    (xtvec.mode === 0 || hadException) ? (xtvec.base @@ U"00") | ((xtvec.base + trapCause) @@ U"00"))
      ...
      decode.arbitration.haltByOther setWhen (stagesFromExecute.map(s =>
        s.arbitration.isValid && s.input(ENV_CTRL) === EnvCtrlEnum.XRET
      ).asBits.orR)

      execute plug new Area {
        import execute._
        def previousStage = decode
        val blockedBySideEffects = stagesFromExecute.tail.map(s => s.arbitration.isValid).asBits().orR || pipeline.service(
          classOf[HazardService]
        ).hazardOnExecuteRS // && s.input(HAS_SIDE_EFFECT)  to improve be less pessimistic
        ...
        val writeData =
          if (noCsrAlu) writeSrc
          else input(INSTRUCTION)(13).mux(
            False -> writeSrc,
...
//        }
        ...
        Component.current.afterElaboration {
          def doJobs(jobs: ArrayBuffer[Any]): Unit = {
            val withWrite =
              jobs.exists(j => j.isInstanceOf[CsrWrite] || j.isInstanceOf[CsrOnWrite] || j.isInstanceOf[CsrDuringWrite])
          ...
          def doJobsOverride(jobs: ArrayBuffer[Any]): Unit = {
            for (element <- jobs) element match {
              case element: CsrReadToWriteOverride if element.that.getBitsWidth != 0 =>
                readToWriteData(element.bitOffset, element.that.getBitsWidth bits) := element.that.asBits
              case _ =>
            }
          }
          ...
          csrOhDecoder match {
            ...
            case true => {
              val oh = csrMapping.mapping.keys.toList.distinct.map(address =>
                address -> RegNextWhen(
                  decode.input(INSTRUCTION)(csrRange) === address,
                  !execute.arbitration.isStuck
                ).setCompositeName(this, "csr_" + address)
              ).toMap
              ...
              for ((address, jobs) <- csrMapping.mapping) {
                ...
                if (jobs.exists(_.isInstanceOf[CsrRead])) {
                  ...
                  when(oh(address))(for (element <- jobs) element match {
                    case element: CsrRead if element.that.getBitsWidth != 0 =>
                      masked(element.bitOffset, element.that.getBitsWidth bits) := element.that.asBits
                    case _ =>
                  })
                  readDatas += masked
                }
