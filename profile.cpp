...
//if(mTime == mTimeCmp) printf("SIM timer tick\n");
				...
				if (riscvRefEnable)
				{
					...
					{
						if (riscvRefEnable)
							riscvRef.trap(true, top->VexRiscv->CsrPlugin_interrupt_code);
					}
				}
#endif
				if (top->VexRiscv->lastStageIsFiring)
				{
					...
					if (riscvRefEnable)
							...
							(rfWriteValid && (rfWriteAddress != riscvRef.rfWriteAddress || rfWriteData != riscvRef.rfWriteData)))
						{
							...
							if (rfWriteValid)
								cout << " REF: RF[" << riscvRef.rfWriteAddress << "] = 0x" << hex << riscvRef.rfWriteData << dec << endl;
							if (rfWriteValid)
								cout << " DUT: RF[" << rfWriteAddress << "] = 0x" << hex << rfWriteData << dec << endl;
							fail();
						}
				}
		...
		catch (const std::exception &e)
		{
			...
			if (riscvRefEnable)
				cout << hex << " REF PC=" << riscvRef.lastPc << " REF I=" << riscvRef.lastInstruction << dec;
			cout << " time=" << i;
			cout << endl;
...
public:
	...
	void sendRsp(uint32_t data)
	{
		if (clientHandle != -1)
		{
			if (send(clientHandle, &data, 4, 0) == -1)
				connectionReset();
		}
	}
};
...
#ifndef REF
#define testA1ReagFileWriteRef \
	{1, 10}, {2, 20}, {3, 40}, { 4, 60 }
#define testA2ReagFileWriteRef \
	{5, 1}, { 7, 3 }
...
public:
	...
	void clientThread()
	{
		...
		while (resetDone != true)
		{
			usleep(100);
		}

		while ((readCmd(2, debugAddress) & RISCV_SPINAL_FLAGS_HALT) == 0)
		{
			usleep(100);
		}
...
public:
	...
	virtual void dBusAccess(uint32_t addr, bool wr, uint32_t size, uint32_t mask, uint32_t *data, bool *error)
	{
		if (isPerifRegion(addr))
			...
			case 0xFFFFFFFC:
				fail();
				break; //Simulation end
			default:
				cout << "Unmapped peripheral access : addr=0x" << hex << addr << " wr=" << wr << " mask=0x" << mask << " data=0x" << data << dec << endl;
				fail();
				break;
			}
...
public:
	...
	virtual void onStdout(char c)
	{
		...
		{
		case LOGIN:
			if (pendingLineContain("buildroot login:"))
			{
				pushCin("root\n");
				state = ECHO_FILE;
			}
			break;
		case ECHO_FILE:
			if (pendingLineContain("# "))
			{
				pushCin("echo \"miaou\" > test.txt\n");
				state = HEXDUMP;
				pendingLine = "";
			}
			break;
		case HEXDUMP:
			if (pendingLineContain("# "))
			{
				pushCin("hexdump -C test.txt\n");
				state = HEXDUMP_CHECK;
				pendingLine = "";
			}
			break;
		case HEXDUMP_CHECK:
			if (pendingLineContain("00000000  6d 69 61 6f 75 0a  "))
			{
				pushCin("");
				state = PASS;
				pendingLine = "";
...
#define redo(count, that)                      \
	for (uint32_t xxx = 0; xxx < count; xxx++) \
	that
#include <pthread.h>
#include <queue>
#include <functional>
